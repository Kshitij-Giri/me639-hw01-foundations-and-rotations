...
class Hw01TfBroadcaster(Node):
    ...
    def keyboard_loop(self):
        while rclpy.ok():
            key = input("Press 'c' for current frame or 'f' for fixed frame: ").strip().lower()
            ...
            if key == "c":
                ...
                print("Composition mode: CURRENT")
    ...
    def on_timer(self):
        ...
        # Get the next elemental rotation from the sequence.
        axis, angle = STEP_SEQUENCE[self.step_index % len(STEP_SEQUENCE)]

        # Construct the elemental rotation matrix.
        R_step = ELEMENTARY_ROTATIONS[axis](angle)

        # Read the current composition mode from the ROS parameter.
        frame = self.get_parameter("compose_frame").value

        # Compose the rotation about either the current body frame
        # or the fixed space frame.
        if frame == "current":
            # Current/body frame: right multiplication
            self.R_body = self.R_body @ R_step

        elif frame == "fixed":
            # Fixed/space frame: left multiplication
            self.R_body = R_step @ self.R_body

        else:
            self.get_logger().warn(
                f"Invalid compose_frame '{frame}'. "
                "Use 'current' or 'fixed'."
            )
            return

        # Move to the next elemental rotation.
        self.step_index += 1

        # Broadcast the updated body frame.
